...
- ❌ **deno_core API stability**: `deno_core` is maintained by the Deno team, and its API surface does not have the same stability guarantees as a 1.0 library. Deno itself can introduce breaking changes to `deno_core` between releases. Forge pins to a specific `deno_core` version and updates deliberately. As of 2026-02-27, the crate source moved from the standalone `github.com/denoland/deno_core` repository (now frozen) into the main Deno monorepo at `github.com/denoland/deno` under `libs/core/`. The crate name, `JsRuntime` API, and embedding model are unchanged; embedding examples are now at `libs/core/examples/`. Latest version: `0.395.0` (2026-03-27).
